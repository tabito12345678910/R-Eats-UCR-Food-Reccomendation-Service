...
void UserPreference::PrintMeals(){
    ...
    for(int i = 0; i<restaurants.getRestaurants().size(); ++i){
        ...
        if(count == 3){
            //loop scans to find all meals that matches users preferences and prints them for non buffet restaurants
            if(!restaurants.getRestaurant(i).getMeal().empty()){
                ...
                if(CheckIfCoffeeOrTeaWanted()){
                    cout<<endl<<"***Prices/Calories for the Coffee/Tea are for the medium sizes.***"<<endl;
                }
                cout<<endl<<endl;
            }
            //buffet restaurants
                ...
                //excludes buffet restaurants when wanting coffee or tea because buffet restaurants dont have coffee or tea
    ...
    if(includesMeal==false){ //no meal matches user's preferences
        cout<<"There are no meals on campus that match your preferences :("<<endl<<endl;
    }
}
    ...
    if(item.getChoice1()==pref1){
        if(item.getChoice2()==pref2){
            if(item.getChoice3()==pref3){
                ...
                cout<<item.getItemName()<<" Price: "<<item.getItemCost()<<", Calories: "<<item.getCalories()<<endl<<endl;
