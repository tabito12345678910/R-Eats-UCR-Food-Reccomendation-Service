...
int main() {
  ...
  while (true) {
    ...
    if (from == "hub" || from == "ssc" || from == "mse" || from == "nd" ||
        from == "sproul" || from == "dorms" || from == "lot30" ||
  ...
  while (true) {
    ...
    UserPreference user =
        UserPreference(preferences.at(0), preferences.at(1), preferences.at(2));
    vector<Restaurant> menu = vRestaurants.getRestaurants();
    while (restaurantChoice != "quit" && restaurantChoice != "q" &&
           restaurantChoice != "again") {
      ...
      cin.clear();
      cin.ignore(256, '\n');
      std::transform(restaurantChoice.begin(), restaurantChoice.end(),
                     restaurantChoice.begin(),
                     [](unsigned char c) { return std::tolower(c); });
      while (restaurantChoice != "quit" && restaurantChoice != "q" &&
             restaurantChoice != "again") {

        if (vRestaurants.isValid(menu, ChronicTacos()) &&
            ChronicTacos().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, ChronicTacos());
        } else if (vRestaurants.isValid(menu, CoffeeBean()) &&
                   CoffeeBean().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, CoffeeBean());
        } else if (vRestaurants.isValid(menu, Habit()) &&
                   Habit().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, Habit());
        } else if (vRestaurants.isValid(menu, HalalShack()) &&
                   HalalShack().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, HalalShack());
        } else if (vRestaurants.isValid(menu, Hibachi()) &&
                   Hibachi().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, Hibachi());
        } else if (vRestaurants.isValid(menu, PandaExpress()) &&
                   PandaExpress().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, PandaExpress());
        } else if (vRestaurants.isValid(menu, Starbucks()) &&
                   Starbucks().isOptionValid(restaurantChoice)) {
          endStatus = learnMore(from, Starbucks());
        } else if (vRestaurants.isValid(menu, Subway()) &&
                   Subway().isOptionValid(restaurantChoice)) {
